...
is made or until `wallstreetbetsd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `wsbc-wallet-cli` and
`wsbc-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--proxy` which tells `wallstreetbetsd` the ip/port of a socks proxy provided by a
separate process. On most systems the configuration will look like:

> `--proxy tor,127.0.0.1:9050,10`
> `--proxy i2p,127.0.0.1:9000`

which tells `wallstreetbetsd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `wallstreetbetsd` the inbound address, network
type, and max connections:

> `--anonymous-inbound rveahdfho7wo4b2m.onion:29996,127.0.0.1:29996,25`
> `--anonymous-inbound cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000,127.0.0.1:30000`

which tells `wallstreetbetsd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:29996" and forwarded to `wallstreetbetsd`
...
forwarded to `wallstreetbetsd` localhost port 30000.
...
`wallstreetbetsd` RPC port - which is independent from the configuration for incoming
...
An ISP can passively monitor `wallstreetbetsd` connections from a node and observe when
...
Run `wallstreetbetsd` as often as possible to conceal when transactions are being sent.
